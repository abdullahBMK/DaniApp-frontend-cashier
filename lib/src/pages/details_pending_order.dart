...
class DetailsPendingOrder extends StatefulWidget {
  RouteArgument routeArgument;

  DetailsPendingOrder({Key key, this.routeArgument}) : super(key: key);

  @override
  _DetailsPendingOrderState createState() {
    return _DetailsPendingOrderState();
  }
}

class _DetailsPendingOrderState extends StateMVC<DetailsPendingOrder> {
  OrderDetailsController _con;
  String value = '';
  _DetailsPendingOrderState() : super(OrderDetailsController()) {
    _con = controller;
  }
                                      ...
                                      child: FlatButton(
                                        onPressed: () {
                                          showDialog(
                                              ...
                                              builder: (context) {
                                                return AlertDialog(
                                                  title: Text(S.of(context).get_order_confirmation_confirmation),
                                                  content: Text(S
                                                      .of(context)
                                                      .would_you_please_confirm_if_you_want_this_order), 
                                                  actions: <Widget>[
                                                    // usually buttons at the bottom of the dialog
                                                    FlatButton(
                                                      child: new Text(S.of(context).confirm),
                                                      onPressed: () {
                                                        // String id = _con.order.id;
                                                        Navigator.of(context).pop();
                                                        _con.dotakeThisOrder(_con.order);
                                                      },
                                                    ),
                                                    FlatButton(
                                        ...
                                        shape: StadiumBorder(),
                                        child: Text(
                                          S.of(context).take_this_order.toUpperCase(),

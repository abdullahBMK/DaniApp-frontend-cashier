...
Future<Stream<Notification>> getNotifications() async {
  ...
  final String url =
   ...
   '${GlobalConfiguration().getString('api_base_url')}notifications?${_apiToken}search=notifiable_id:${_user.id}&searchFields=notifiable_id:=&orderBy=created_at&sortedBy=desc&limit=30';
  print(url);
...
Future<Notification> markAsReadNotifications(Notification notification) async {
  ...
  print(url);
  print("[${response.body}] NotificationRepository markAsReadNotifications");
